...
NONSMOOTH_ACTIVATIONS = ["AppReLU"]
ACTIVATION_NAME = "AppReLU"
...
def _get_module(**kw):
    return torch_activation.AppReLU(**kw)
...
class TestNumerical:
    ...
    def test_power_shape(self):
        m = _get_module(a=2.0, b=2.0)
        x = torch.tensor([0.0, 1.0, 2.0, -1.0])
        expected = torch.tensor([0.0, 2.0, 8.0, 0.0])
        assert torch.allclose(m(x), expected, atol=1e-5)

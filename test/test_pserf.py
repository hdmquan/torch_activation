...
ACTIVATION_NAME = "PSerf"


def scalar_ref(x: float) -> float:
    return x * math.erf(1.0 * math.log(1 + math.exp(min(1.0 * x, 20))))
...
class TestEdgeCases:
    def test_no_nan_inf(self):
        ...
        for val in [0.0, 1e3, -1e3]:
            x = torch.full((4,), val)
            out = m(x)
            assert not torch.isnan(out).any(), f"NaN in output for x={val}"
            assert not torch.isinf(out).any(), f"Inf in output for x={val}"
